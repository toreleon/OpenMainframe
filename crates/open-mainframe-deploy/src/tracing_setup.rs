...
impl Default for TracingConfig {
    fn default() -> Self {
        Self {
            ...
            otel_endpoint: None,
            service_name: "open-mainframe".to_string(),
            sample_rate: 0.1,
        }
    }
}
...
impl TracingConfig {
    ...
    pub fn from_env() -> Self {
        Self {
            log_level: env::var("OPEN_MAINFRAME_LOG_LEVEL").unwrap_or_else(|_| "info".to_string()),
            log_format: env::var("OPEN_MAINFRAME_LOG_FORMAT")
                .map(|f| LogFormat::from_str(&f))
                .unwrap_or(LogFormat::Json),
            otel_endpoint: env::var("OTEL_EXPORTER_OTLP_ENDPOINT").ok(),
            service_name: env::var("OTEL_SERVICE_NAME")
                .unwrap_or_else(|_| "open-mainframe".to_string()),
