//! Configuration management for OpenMainframe.
...
impl Config {
    ...
    pub fn load() -> Result<Self, ConfigError> {
        ...
        if let Ok(path) = env::var("OPEN_MAINFRAME_CONFIG_PATH") {
            config = Self::load_from_file(&path)?;
        } else if Path::new("/etc/open-mainframe/config.yaml").exists() {
            config = Self::load_from_file("/etc/open-mainframe/config.yaml")?;
        }
    ...
    fn apply_env_overrides(&mut self) {
        if let Ok(host) = env::var("OPEN_MAINFRAME_SERVER_HOST") {
            self.server.host = host;
        }
        if let Ok(port) = env::var("OPEN_MAINFRAME_SERVER_PORT") {
            if let Ok(p) = port.parse() {
                self.server.port = p;
            }
        }
        if let Ok(port) = env::var("OPEN_MAINFRAME_METRICS_PORT") {
            if let Ok(p) = port.parse() {
                self.server.metrics_port = p;
            }
        }
        if let Ok(url) = env::var("OPEN_MAINFRAME_DB_URL") {
            self.database.url = url;
        }
        if let Ok(level) = env::var("OPEN_MAINFRAME_LOG_LEVEL") {
            self.observability.log_level = level;
        }
        if let Ok(format) = env::var("OPEN_MAINFRAME_LOG_FORMAT") {
...
impl Default for DatabaseConfig {
    fn default() -> Self {
        Self {
            url: "postgresql://localhost:5432/openmainframe".to_string(),
            pool_size: 10,
            idle_timeout: 300,
            max_lifetime: 3600,
        }
...
impl Default for ObservabilityConfig {
    fn default() -> Self {
        Self {
            ...
            otel_endpoint: None,
            metrics_prefix: "open_mainframe".to_string(),
            sample_rate: 0.1,
        }
    }
}
