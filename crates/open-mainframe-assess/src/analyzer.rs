            ...
            match feature.category {
                FeatureCategory::Database => {
                    if feature.name == "DB2" {
                        recommendations.push(
                            "Consider using PostgreSQL with the open-mainframe-db2 compatibility layer".to_string()
                        );
                    }
                }
                FeatureCategory::Transaction => {
                    if feature.name == "CICS" {
                        recommendations.push(
                            "CICS commands can be emulated using the open-mainframe-cics runtime".to_string()
                        );
                    }
                }
                FeatureCategory::FileHandling => {
                    if feature.name == "VSAM" {
                        recommendations.push(
                            "VSAM files can be migrated to the open-mainframe-dataset VSAM emulation".to_string()
                        );
                    }
                }
                _ => {}
