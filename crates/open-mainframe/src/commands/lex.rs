...
pub fn run(input: PathBuf, format: String) -> Result<()> {
    ...
    let source_format = match format.to_lowercase().as_str() {
        "fixed" => open_mainframe_cobol::SourceFormat::Fixed,
        "free" => open_mainframe_cobol::SourceFormat::Free,
        _ => {
                ...
                .unwrap_or(false)
            {
                ...
                open_mainframe_cobol::SourceFormat::Free
            } else {
    ...
    // Create source file
    let source_file =
        open_mainframe_cobol::SourceFile::from_text(open_mainframe_cobol::FileId::MAIN, source, source_format);

    // Scan the source file
    let (tokens, errors) = open_mainframe_cobol::scan(&source_file);
