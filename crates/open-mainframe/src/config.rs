//! Configuration system for OpenMainframe.
//!
//! Supports loading configuration from:
//! - `open-mainframe.toml` in current directory
//! - `~/.config/open-mainframe/config.toml` for user defaults
//! - Environment variables (OPEN_MAINFRAME_*)
...
/// Complete configuration for OpenMainframe.
...
impl Config {
    ...
    pub fn load() -> Self {
        ...
        if let Ok(c) = Self::load_from_file(Path::new("open-mainframe.toml")) {
            config = config.merge(c);
        }
    ...
    pub fn user_config_path() -> Option<PathBuf> {
        dirs::config_dir().map(|d| d.join("open-mainframe").join("config.toml"))
    }
    ...
    fn apply_env_vars(&mut self) {
        if let Ok(val) = std::env::var("OPEN_MAINFRAME_SOURCE_FORMAT") {
            self.compiler.source_format = val;
        }
        if let Ok(val) = std::env::var("OPEN_MAINFRAME_COPYBOOK_PATH") {
            self.compiler.copybook_paths = val.split(':').map(PathBuf::from).collect();
        }
        if let Ok(val) = std::env::var("OPEN_MAINFRAME_OPTIMIZATION") {
            if let Ok(level) = val.parse() {
                self.compiler.optimization = level;
            }
        }
        if let Ok(val) = std::env::var("OPEN_MAINFRAME_DATASET_DIR") {
            self.dataset.base_dir = PathBuf::from(val);
        }
        if let Ok(val) = std::env::var("OPEN_MAINFRAME_PROGRAM_DIR") {
            self.jcl.program_dir = PathBuf::from(val);
        }
        if let Ok(val) = std::env::var("OPEN_MAINFRAME_WORK_DIR") {
